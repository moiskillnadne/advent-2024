...
for (let level of processedInput) {
    ...
    const levelSafety = validateLevel(level)

    const isLevelSafe = levelSafety.every((gapCheck) => gapCheck === true)
    ...
    const failIndex = levelSafety.findIndex((value) => value === false)

    const levelWithoutFailIndex = level.toSpliced(failIndex, 1)

    const levelWithoutIndexSafety = validateLevel(levelWithoutFailIndex)

    const isLevelWithoutIndexSafe = levelWithoutIndexSafety.every((gapCheck) => gapCheck === true)

    if(isLevelWithoutIndexSafe) {
        safeLevels += 1
        continue
    }

    const preFailedIndex = failIndex - 1;

    if(preFailedIndex >= 0) {
        const levelWithoutPreFailIndex = level.toSpliced(preFailedIndex, 1)
        const levelWithoutPreIndexSafety = validateLevel(levelWithoutPreFailIndex)

        const isLevelWithoutPreIndexSafe = levelWithoutPreIndexSafety.every((gapCheck) => gapCheck === true)

        if(isLevelWithoutPreIndexSafe) {
            safeLevels += 1
            continue
        }
    }

    const afterFailedIndex = failIndex + 1;

    if(afterFailedIndex < level.length) {
        const levelWithoutAfterFailIndex = level.toSpliced(afterFailedIndex, 1)
        const levelWithoutAfterIndexSafety = validateLevel(levelWithoutAfterFailIndex)

        const isLevelWithoutAfterIndexSafe = levelWithoutAfterIndexSafety.every((gapCheck) => gapCheck === true)

        if(isLevelWithoutAfterIndexSafe) {
            safeLevels += 1
        }
    }
...
console.log([1, 2, 3, 4, 5, 6].toSpliced(1, 1))

// 564 level - first result